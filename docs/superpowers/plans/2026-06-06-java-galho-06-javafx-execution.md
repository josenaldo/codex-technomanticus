...
8. `## Veja também` — wikilinks SEM backticks, SEM âncoras same-file `[[#...]]`. Sempre: notas do galho + `[[03-Dominios/Java/JavaFX/index|JavaFX (MOC do galho)]]` + `[[03-Dominios/Java/index|Trilha Java]]` + (quando conectar) Galhos 3/4/5 + verbetes do Dicionário.
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/` (pasta)
...
```bash
mkdir -p "03-Dominios/Java/JavaFX"
```

- [ ] **Step 3: Reconfirmar o tronco e a fabricação (contraexemplo)**

```bash
grep -nE "Patient|Josenaldo|minha experiência" "03-Dominios/Java/Frontend/JavaFX.md"
```
...
```bash
ls "03-Dominios/Java/Swing/" | grep -E "^(03|04|05|06|08|10|12) "
ls "03-Dominios/Java/JVM/" | grep -E "^08 "
ls "03-Dominios/Java/Concorrência e paralelismo/" | grep -E "^12 "
...
```bash
grep -rni "javafx" "03-Dominios/Java/Swing/" "03-Dominios/Java/JVM/" "03-Dominios/Java/Linguagem e sintaxe moderna/" "03-Dominios/Java/Collections e Streams/" "03-Dominios/Java/Concorrência e paralelismo/" | grep -i "planejado"
grep -rn "\[\[JavaFX\]\]" "03-Dominios/Java/" --include="*.md" | grep -v "Frontend/JavaFX.md"
```
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/01 - JavaFX — o que é e como chega ao projeto.md`
...
- `## Em entrevista` + `## Veja também` ([[02 - Scene graph — stage, scene e nodes]], [[13 - Empacotamento — módulos, jlink e jpackage]], [[14 - JavaFX hoje — estado do projeto e Swing vs JavaFX]], [[03-Dominios/Java/Swing/12 - Swing hoje - estado atual|Swing hoje]], MOC galho, MOC central, [[03-Dominios/Java/Dicionário de Java#OpenJFX|OpenJFX (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#Stage / Scene|Stage / Scene (Dicionário)]]) + `## Referências`.
...
```bash
grep -cE "^## (O que é|Por que importa|Como funciona|Na prática|Armadilhas|Em entrevista|Veja também|Referências)" "03-Dominios/Java/JavaFX/01 - JavaFX — o que é e como chega ao projeto.md"
grep -E "org.openjfx|Java 11|OpenJFX|Application|Stage" "03-Dominios/Java/JavaFX/01 - JavaFX — o que é e como chega ao projeto.md" | head
```
...
```bash
git add "03-Dominios/Java/JavaFX/01 - JavaFX — o que é e como chega ao projeto.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/02 - Scene graph — stage, scene e nodes.md`
...
- `## Em entrevista` + `## Veja também` (01, 03, 09, 10, 12, [[03-Dominios/Java/Swing/10 - Custom painting e componentes customizados|Custom painting (Swing)]], MOC, central, [[03-Dominios/Java/Dicionário de Java#scene graph|scene graph (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#retained mode / immediate mode|retained mode (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "Node|Parent|Group|Region|retained|coordenada|transform" "03-Dominios/Java/JavaFX/02 - Scene graph — stage, scene e nodes.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/02 - Scene graph — stage, scene e nodes.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/03 - Layout panes.md`
...
- `## Em entrevista` + `## Veja também` (02, 04, 06, 09, [[03-Dominios/Java/Swing/03 - Layout managers|Layout managers (Swing)]], MOC, central, [[03-Dominios/Java/Dicionário de Java#layout pane|layout pane (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "VBox|HBox|BorderPane|GridPane|StackPane|AnchorPane|Hgrow|Priority" "03-Dominios/Java/JavaFX/03 - Layout panes.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/03 - Layout panes.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/04 - Controls essenciais.md`
...
- `## Em entrevista` + `## Veja também` (03, 05, 06, 07, 08, MOC, central, [[03-Dominios/Java/Dicionário de Java#ObservableList|ObservableList (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "Button|TextField|ComboBox|CheckBox|ToggleGroup|ListView|TableView|Alert" "03-Dominios/Java/JavaFX/04 - Controls essenciais.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/04 - Controls essenciais.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/05 - Eventos — capturing, bubbling e handlers.md`
...
- `## Em entrevista` + `## Veja também` (02, 04, 10, [[03-Dominios/Java/Swing/04 - O modelo de eventos|O modelo de eventos (Swing)]], [[03-Dominios/Java/Collections e Streams/04 - Lambdas e interfaces funcionais|Lambdas]], MOC, central, [[03-Dominios/Java/Dicionário de Java#event dispatch chain (capturing / bubbling)|event dispatch chain (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#EventFilter / EventHandler|EventFilter / EventHandler (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "capturing|bubbling|EventFilter|EventHandler|consume|dispatch chain|setOnAction" "03-Dominios/Java/JavaFX/05 - Eventos — capturing, bubbling e handlers.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/05 - Eventos — capturing, bubbling e handlers.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/06 - FXML e Scene Builder.md`
...
- `## Em entrevista` + `## Veja também` (01, 04, 07, 11, MOC, central, [[03-Dominios/Java/Dicionário de Java#FXML|FXML (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#fx:id / @FXML|fx:id (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#FXMLLoader|FXMLLoader (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#Scene Builder|Scene Builder (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "fx:controller|fx:id|@FXML|initialize|FXMLLoader|ResourceBundle|Scene Builder" "03-Dominios/Java/JavaFX/06 - FXML e Scene Builder.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/06 - FXML e Scene Builder.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/07 - Properties e binding.md`
...
- `## Em entrevista` + `## Veja também` (04, 06, 08, 10, 11, MOC, central, [[03-Dominios/Java/Dicionário de Java#Property (JavaFX)|Property (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#binding (JavaFX)|binding (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "ObservableValue|SimpleStringProperty|bindBidirectional|invalidation|change listener|Bindings|Weak" "03-Dominios/Java/JavaFX/07 - Properties e binding.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/07 - Properties e binding.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/08 - TableView, cell factories e dados observáveis.md`
...
- `## Em entrevista` + `## Veja também` (04, 07, 09, [[03-Dominios/Java/Swing/08 - Renderers e editors|Renderers e editors (Swing)]], MOC, central, [[03-Dominios/Java/Dicionário de Java#cell factory / cell value factory|cell factory (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#ObservableList|ObservableList (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "cellValueFactory|cellFactory|updateItem|ObservableList|FilteredList|SortedList|comparatorProperty" "03-Dominios/Java/JavaFX/08 - TableView, cell factories e dados observáveis.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/08 - TableView, cell factories e dados observáveis.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/09 - CSS em JavaFX.md`
...
- `## Em entrevista` + `## Veja também` (02, 03, 08, 12, [[03-Dominios/Java/Swing/09 - Look and Feel e temas|Look and Feel (Swing)]], MOC, central, [[03-Dominios/Java/Dicionário de Java#CSS do JavaFX (-fx-)|CSS do JavaFX (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#Modena|Modena (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "\-fx\-|Modena|pseudo|stylesheet|precedência|getStyleClass" "03-Dominios/Java/JavaFX/09 - CSS em JavaFX.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/09 - CSS em JavaFX.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/10 - A JavaFX Application Thread — Task, Service e Platform.runLater.md`
...
- `## Em entrevista` + `## Veja também` (05, 07, 08, [[03-Dominios/Java/Swing/05 - A Event Dispatch Thread|EDT (Swing)]], [[03-Dominios/Java/Swing/06 - SwingWorker e tarefas em background|SwingWorker (Swing)]], [[03-Dominios/Java/Concorrência e paralelismo/12 - Virtual Threads e Project Loom|Virtual Threads]], MOC, central, [[03-Dominios/Java/Dicionário de Java#JavaFX Application Thread|JavaFX Application Thread (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#Platform.runLater|Platform.runLater (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#Task / Service (JavaFX)|Task / Service (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "Application Thread|Platform.runLater|Task|Service|updateProgress|setOnSucceeded|setOnFailed|isCancelled" "03-Dominios/Java/JavaFX/10 - A JavaFX Application Thread — Task, Service e Platform.runLater.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/10 - A JavaFX Application Thread — Task, Service e Platform.runLater.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/11 - Arquitetura — MVC, MVVM e injeção de dependência.md`
...
- `## Em entrevista` + `## Veja também` (06, 07, 10, 14, MOC, central, [[03-Dominios/Java/Dicionário de Java#MVVM|MVVM (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "MVVM|ViewModel|controllerFactory|testáve|binda" "03-Dominios/Java/JavaFX/11 - Arquitetura — MVC, MVVM e injeção de dependência.md" | head
grep -E "\[\[[^]]*Spring" "03-Dominios/Java/JavaFX/11 - Arquitetura — MVC, MVVM e injeção de dependência.md"
```
...
```bash
git add "03-Dominios/Java/JavaFX/11 - Arquitetura — MVC, MVVM e injeção de dependência.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/12 - Custom controls, Canvas e charts.md`
...
- `## Em entrevista` + `## Veja também` (02, 09, [[03-Dominios/Java/Swing/10 - Custom painting e componentes customizados|Custom painting (Swing)]], MOC, central, [[03-Dominios/Java/Dicionário de Java#Canvas (JavaFX)|Canvas (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#skin (Control)|skin (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "Canvas|GraphicsContext|Skin|Control|LineChart|XYChart|Region" "03-Dominios/Java/JavaFX/12 - Custom controls, Canvas e charts.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/12 - Custom controls, Canvas e charts.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/13 - Empacotamento — módulos, jlink e jpackage.md`
...
- `## Em entrevista` + `## Veja também` (01, 11, 14, [[03-Dominios/Java/JVM/08 - JPMS — o sistema de módulos|JPMS (Galho 3)]], MOC, central, [[03-Dominios/Java/Dicionário de Java#jlink|jlink (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#jpackage|jpackage (Dicionário)]]) + `## Referências`.
...
```bash
grep -E "javafx.controls|javafx.fxml|module-path|add-modules|jlink|jpackage|JEP 392|runtime components" "03-Dominios/Java/JavaFX/13 - Empacotamento — módulos, jlink e jpackage.md" | head
```

- [ ] **Step 4: Commit**

```bash
git add "03-Dominios/Java/JavaFX/13 - Empacotamento — módulos, jlink e jpackage.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/14 - JavaFX hoje — estado do projeto e Swing vs JavaFX.md`
...
- `## Veja também` — (01, 07, 10, 11, 13, [[03-Dominios/Java/Swing/12 - Swing hoje - estado atual|Swing hoje (Galho 5)]], [[03-Dominios/Java/Swing/index|Swing (MOC do Galho 5)]], MOC, central, [[03-Dominios/Java/Dicionário de Java#OpenJFX|OpenJFX (Dicionário)]], [[03-Dominios/Java/Dicionário de Java#Gluon|Gluon (Dicionário)]]) + `## Referências`.
...
```bash
grep -iE "decision|matrix|Gluon|OpenJFX|cadence|cheatsheet" "03-Dominios/Java/JavaFX/14 - JavaFX hoje — estado do projeto e Swing vs JavaFX.md" | head
grep -riE "minha experiência|no meu projeto|josenaldo|Patient|% dos desenvolvedores|market share" "03-Dominios/Java/JavaFX/14 - JavaFX hoje — estado do projeto e Swing vs JavaFX.md"
```
...
```bash
git add "03-Dominios/Java/JavaFX/14 - JavaFX hoje — estado do projeto e Swing vs JavaFX.md"
...
**Files:**
- Create: `03-Dominios/Java/JavaFX/index.md`
...
Conteúdo (modelar pelo `03-Dominios/Java/JVM/index.md`):
...
- `## Sobre este galho` — escopo + audiência + **refator com poda integral** do tronco `Frontend/JavaFX.md` + par desktop com [[03-Dominios/Java/Swing/index|Swing (Galho 5)]] + fronteiras (teoria de UI thread → Galho 5; JPMS → Galho 3; concorrência → Galho 4).
...
- `## Todas as notas` — Dataview (`FROM "03-Dominios/Java/JavaFX"`, `WHERE type = "concept"`).
- `## Veja também` — MOC central, Galhos 1/3/4/5 (index), Dicionário, `[[JavaFX]]` (tronco em transição).

- [ ] **Step 2: Verificar**

```bash
grep -cE "^## (Iniciado|Adepto|Magus|Rotas alternativas)" "03-Dominios/Java/JavaFX/index.md"
grep -c "\[\[" "03-Dominios/Java/JavaFX/index.md"
```
...
```bash
git add "03-Dominios/Java/JavaFX/index.md"
...
```bash
grep -rhoE "Dicionário de Java#[^]|]+" "03-Dominios/Java/JavaFX/" | sort -u
```
...
```bash
grep -E "^### (scene graph|FXML|Property \(JavaFX\)|jpackage|MVVM|Gluon)" "03-Dominios/Java/Dicionário de Java.md"
...
```bash
grep -E "JavaFX/index" "03-Dominios/Java/index.md"
...
**Files:**
- Modify: `03-Dominios/Java/Frontend/JavaFX.md`
...
```bash
grep -nE "Patient|Josenaldo|minha experiência|is my choice" "03-Dominios/Java/Frontend/JavaFX.md"
grep -c "Migrado para galho próprio" "03-Dominios/Java/Frontend/JavaFX.md"
grep -E "publish: false" "03-Dominios/Java/Frontend/JavaFX.md"
wc -l "03-Dominios/Java/Frontend/JavaFX.md"
```
...
```bash
git add "03-Dominios/Java/Frontend/JavaFX.md"
...
```bash
grep -rni "javafx" "03-Dominios/Java/Swing/" "03-Dominios/Java/JVM/" "03-Dominios/Java/Concorrência e paralelismo/" "03-Dominios/Java/Collections e Streams/" "03-Dominios/Java/Linguagem e sintaxe moderna/" | grep -iE "planejado|galho 6"
```

- [ ] **Step 2: Converter cada ponteiro** pro wikilink mais específico, ajustando o texto pra não dizer mais "planejado":
- Menção a comparação/estado ("Swing vs JavaFX", capstone do Swing) → `[[03-Dominios/Java/JavaFX/14 - JavaFX hoje — estado do projeto e Swing vs JavaFX|JavaFX hoje]]`
- Menção genérica ao galho (MOCs) → `[[03-Dominios/Java/JavaFX/index|JavaFX (Galho 6)]]`
...
```bash
grep -rni "javafx" "03-Dominios/Java/Swing/" "03-Dominios/Java/JVM/" "03-Dominios/Java/Concorrência e paralelismo/" | grep -i "planejado"
```
Expected: VAZIO.
...
```bash
ls "03-Dominios/Java/JavaFX/" | sort
```
...
```bash
for f in "03-Dominios/Java/JavaFX/"[0-9]*.md; do grep -H "^fase:" "$f"; done
```

- [ ] **Step 3: Seções obrigatórias (3 por nota)**

```bash
for f in "03-Dominios/Java/JavaFX/"[0-9]*.md; do echo "$f: $(grep -cE '^## (Em entrevista|Armadilhas|Veja também)' "$f")"; done
```

- [ ] **Step 4: Anti-fabricação + wikilinks proibidos**

```bash
grep -riE "minha experiência|no meu projeto|josenaldo|Patient|getSpecialty|market share|% dos desenvolvedores" "03-Dominios/Java/JavaFX/"
grep -rE "\[\[[^]]*(Spring|Jakarta|GraalVM|Gluon Mobile|Galho 1[5-8]|Galho [78])" "03-Dominios/Java/JavaFX/"
grep -rn '\[\[#' "03-Dominios/Java/JavaFX/"
```
...
```bash
for f in "03-Dominios/Java/JavaFX/"[0-9]*.md; do echo "$f: $(grep -c '### Frase pronta (inglês)' "$f")"; done
```

- [ ] **Step 6: Skill `verificar-wikilinks`**

Rodar na pasta `03-Dominios/Java/JavaFX/` + conferir (via report da pasta `03-Dominios/Java` filtrado) os arquivos tocados fora: `index.md` central, Dicionário, `Frontend/JavaFX.md`, arquivos da dívida reversa. Âncoras `Dicionário de Java#...` resolvem 1:1. Corrigir e commitar à parte se houver.
