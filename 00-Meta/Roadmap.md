...
- **Design de Software** — OO ✅ + SOLID ✅. **EM CURSO (2026-07-28):** o antigo "galho de Design Patterns" foi promovido a galho-pai **[[03-Dominios/Engenharia/Design de Software/Padrões de Projeto/roadmap|Padrões de Projeto]]** — um **catálogo de consulta** para o sênior (inclusive legado), com lente cross-linguagem (Java/TS/Python/Go) e peso no "quando NÃO usar". **6 famílias** (Clássicos/GoF · Acesso a Dados · Integração Empresarial/EIP · Aplicação Corporativa/PoEAA · Arquitetura de Eventos · Nuvem e Resiliência), ~90-100 notas, construção **sequencial**. Roadmaps montados; **família 1 Clássicos (GoF) COMPLETA 23/23 (2026-07-28)** — os 23 padrões em 3 fases, lente cross-linguagem, Armadilhas reforçadas, inglês. Falta podar o monólito `Design Patterns.md` + index do galho-pai; depois família 2 (Acesso a Dados). Rosters das 6 famílias no roadmap-pai (anti-drift).
