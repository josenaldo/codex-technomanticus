---
title: "Trilha Entrevistas"
created: 2026-04-01
updated: 2026-04-01
type: concept
progress: backlog
status: seedling
tags:
  - trilha
  - entrevista
publish: false
---

# Trilha Entrevistas

Roteiro de preparação para entrevistas de Senior Fullstack Developer em vagas internacionais remotas. Cada item linka para uma nota detalhada e bilíngue.

> **Como usar:** siga as fases na ordem. Marque `[x]` ao completar cada estudo. Use as seções "How to explain in English" de cada nota para praticar articulação em inglês.

---

## Fase 1 — Fundamentos

> O que qualquer senior precisa articular com clareza.

- [ ] [[Estruturas de Dados]]
- [ ] [[Algoritmos]]
- [ ] [[Orientação a Objetos]]
- [ ] [[Design Patterns]]
- [ ] [[Banco de dados]]
- [ ] [[Testes]]
- [ ] [[Redes e Protocolos]]

## Fase 2 — Arquitetura & System Design

> O diferencial de senior: tomar decisões de arquitetura e justificar.

- [ ] [[System Design]]
- [ ] [[API Design]]
- [ ] [[Arquitetura de Software]]
- [ ] [[Event Storming]]
- [ ] [[Mensageria]]
- [ ] [[Event Streaming]]
- [ ] [[03-Dominios/Tecnologia/Java/Backend/Kafka/Kafka]]
- [ ] [[RabbitMQ]]
- [ ] [[BullMQ]]

## Fase 3 — Stack Java

> Profundidade no backend principal.

- [ ] [[Java Fundamentals]]
- [ ] [[Spring Boot]]
- [ ] [[03-Dominios/Tecnologia/Java/Backend/Kafka/Kafka]]
- [ ] [[JavaFX]]

## Fase 4 — Stack JavaScript/TypeScript

> Fullstack: dominar o ecossistema JS.

- [ ] [[JavaScript Fundamentals]]
- [ ] [[TypeScript]]
- [ ] [[Node.js]]
- [ ] [[React]]
- [ ] [[HTML e CSS]]
- [ ] [[Bootstrap]]
- [ ] [[Material UI]]
- [ ] [[03-Dominios/Tecnologia/React/Mantine]]

## Fase 4.5 — Infraestrutura & DevOps

> O que sustenta tudo em produção.

- [ ] [[Docker]]
- [ ] [[Kubernetes]]
- [ ] [[Nginx]]
- [ ] [[Linux]]
- [ ] [[Terminal]]

## Fase 4.6 — Inteligência Artificial

> IA como ferramenta de produtividade e feature de produto.

- [ ] [[03-Dominios/Tecnologia/IA/index|Inteligência Artificial]]
- [ ] [[Anatomia dos LLMs|LLMs]]
- [ ] [[Anatomia de Agents|Agents]]
- [ ] [[Context Engineering]]
- [ ] [[Claude]]
- [ ] [[GitHub Copilot]]
- [ ] [[Codex]]
- [ ] [[Gemini]]
- [ ] [[Comparativo de LLMs]]

## Fase 5 — Behavioral & Comunicação

> Como se vender em inglês e demonstrar senioridade.

- [ ] [[Behavioral Questions]]
- [ ] [[STAR Method]]
- [ ] [[Communicating Trade-offs]]

## Fase 6 — Prática

> Mock interviews e exercícios.

- [ ] [[System Design Practice]]
- [ ] [[Coding Challenges Strategy]]
