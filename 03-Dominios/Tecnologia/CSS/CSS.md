...
> [!nota] Nota-semente
> Conteúdo migrado do antigo "HTML e CSS" na reorganização em camadas. Cobre **CSS 2026** — box model, layout (Flexbox, Grid, Subgrid), responsividade (container queries), design tokens, animações, specificity, performance e utility-first (Tailwind). Será expandida numa trilha 3-fases (Iniciado/Adepto/Magus) no Batch 5. A marcação vive em [[03-Dominios/Tecnologia/HTML/index|HTML]].

**CSS (Cascading Style Sheets)** é a linguagem de estilo que define a apresentação visual — layout, cores, tipografia, animações. Em 2026, CSS moderno resolve o que antes exigia JavaScript: animações complexas, layouts dinâmicos, estados via `:has`, container queries. Separação clássica: HTML é estrutura, CSS é apresentação, JS é comportamento.
