...
> **O que acontece:** o teste falha intermitentemente, você adiciona `await page.waitForTimeout(2000)` antes da ação, e "resolve" — até voltar a falhar numa CI mais lenta, ou ficar lento demais.
> **Por quê:** `sleep` é uma aposta cega: ou espera de menos (flaky) ou de mais (lento). Ele ignora o auto-waiting que o Playwright já faz de graça.
> **Como evitar:** confie no auto-wait dos locators e nas web-first assertions. Se precisa esperar uma **condição**, espere *a condição* (`await expect(locator).toBeVisible()`, `page.waitForResponse(...)`), nunca um tempo fixo. `waitForTimeout` só em depuração descartável, jamais commitado.
