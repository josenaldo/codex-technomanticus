...
> [!tip] Vídeo — os três papéis, e a divisão de trabalho por trás deles
> [**Kubernetes Volumes explained | Persistent Volume, Persistent Volume Claim & Storage Class**](https://www.youtube.com/watch?v=0swOh5C3OVM) (TechWorld with Nana, ~21 min, EN) cobre o mesmo trio da seção acima e acrescenta o enquadramento **organizacional** que explica por que o modelo tem três peças em vez de uma. O PV é apresentado como recurso de cluster equivalente a CPU ou memória — algo que o administrador provisiona —, enquanto o PVC é o pedido que o time de aplicação escreve sem saber qual armazenamento está por trás. Antes da StorageClass, isso significava um administrador criando PVs à mão **antes** de cada aplicação subir; a StorageClass automatiza esse lado e abstrai o provedor. Duas precisões práticas aparecem e vale reter: a distinção entre volume **local** e **remoto** — e por que backup e sobrevivência a perda de nó dependem dela —, e a regra de que **o PVC precisa existir no mesmo namespace do Pod** que o usa. **O que ele não cobre:** modos de acesso com a precisão que esta nota exige, `volumeBindingMode` e o binding tardio, política de recuperação, expansão de volume, CSI como padrão, e snapshots.
