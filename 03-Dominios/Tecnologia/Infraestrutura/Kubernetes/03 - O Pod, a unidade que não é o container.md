...
> [!tip] Vídeo — vendo o `pause` com os próprios olhos
> [**Pods and Containers — Kubernetes Networking | Container Communication inside the Pod**](https://www.youtube.com/watch?v=5cNrTU6o3Fw) (TechWorld with Nana, ~14 min, EN) percorre a mesma tese desta nota — *a unidade é o Pod, não o container* — e faz na tela a demonstração que fecha a seção anterior. Ele parte do contraste que o leitor vindo de Docker tem na cabeça (no Docker você publica porta do host para porta do container; no Kubernetes o Pod recebe **seu próprio namespace de rede e sua própria interface virtual**), passa pelo padrão sidecar e por `kubectl exec -c` quando há mais de um container, e chega ao ponto alto em [11:46]: ele entra no nó do minikube, roda `docker ps` e mostra a lista de **containers `pause`** rodando ali — um para cada Pod, cada um carregando no nome o Pod a que pertence. É o container invisível da seção acima, visível. **O que ele não cobre:** as fases do `status.phase`, a ordem estrita entre init containers, `restartPolicy` e os padrões de composição além do sidecar — ambassador e adapter não aparecem.
