...
> [!tip] Vídeo — a sucessora, com as duas motivações que a explicam
> [**Gateway API Explained: The Future of Kubernetes Networking**](https://www.youtube.com/watch?v=xaZ87iSvMAI) (KodeKloud, ~45 min, EN) desenvolve com laboratório na tela as duas razões que a seção acima resume. A primeira é a **perda de portabilidade** causada pelas annotations: quem configurou tudo por annotations de um controlador e depois decide trocar de controlador descobre que a configuração não acompanha — ela nunca foi parte da especificação, era dialeto. A segunda é a **separação de papéis**, que o Ingress não modela: o time de infraestrutura passa a ser dono do `GatewayClass` e do `Gateway`, enquanto cada time de aplicação escreve seu próprio `HTTPRoute` e o vincula ao Gateway por `parentRefs` — com o Gateway controlando, via `allowedRoutes`, de quais namespaces ele aceita rotas. Ele também percorre os tipos de rota além de HTTP (`TLSRoute`, `TCPRoute`, `UDPRoute`, `GRPCRoute`), que é a resposta ao limite do Ingress de só saber roteamento HTTP por caminho e host. A parte final é prática, com um Gateway baseado em Nginx sendo instalado e exercitado. **O que ele não cobre:** o Ingress em si com a profundidade desta nota — `IngressClass`, terminação de TLS, hospedagem virtual por nome — e nada do diagnóstico de um Ingress que não funciona.
