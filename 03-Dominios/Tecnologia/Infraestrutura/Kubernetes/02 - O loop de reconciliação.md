...
> A descrição do fluxo `apply` → validação → etcd reflete o comportamento do Kubernetes em versões amplamente usadas em 2026 (linha 1.3x), com *server-side apply* como mecanismo estável desde a 1.22 e habilitado por padrão. Detalhes internos do api-server — como ele decide entre criar e atualizar um objeto, o funcionamento exato do *server-side apply* versus o `apply` tradicional do lado do cliente — pertencem à parte deste galho dedicada ao control plane por dentro, mais adiante na fase Magus, não a esta nota: aqui basta reter que existe uma fronteira síncrona clara, terminando na escrita do etcd.
