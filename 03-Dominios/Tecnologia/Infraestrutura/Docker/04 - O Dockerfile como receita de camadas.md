...
> [!tip] Vídeo — o Dockerfile escrito do zero, com o erro clássico incluído
> [**Dockerfile Tutorial — Docker in Practice**](https://www.youtube.com/watch?v=WmcdMiyqfZs) (TechWorld with Nana, ~24 min, EN) escreve um Dockerfile linha a linha para uma aplicação real e explica cada instrução no momento em que ela entra. O trecho que mais serve a esta nota é o do `FROM`: ela abre a imagem base para mostrar que **ela também é feita de camadas** — Alpine embaixo, Node instalado por cima, e a sua aplicação por cima disso — o que torna concreto o argumento de que não existe "imagem base" como coisa atômica, só uma pilha que alguém já empilhou. O outro momento útil é pedagógico por acidente: ela roda o container e ele falha porque o `server.js` não é encontrado, e a investigação leva ao `WORKDIR` — o tipo de erro que a régua desta nota previne, visto acontecendo. Ela fecha entrando no container para conferir o que de fato foi parar lá dentro, e removendo do `COPY` o que não deveria ter ido. **O que ele não cobre:** a régua "isto cria camada?" aplicada instrução a instrução, `ADD` contra `COPY`, e a ordem das instruções tratada como decisão de arquitetura.
