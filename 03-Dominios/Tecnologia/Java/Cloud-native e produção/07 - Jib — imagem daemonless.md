...
> [!abstract] TL;DR
> **Jib** (`com.google.cloud.tools:jib`, do Google, v3.5.x) builda imagem OCI/Docker **sem Docker daemon e sem Dockerfile**, direto de um plugin Maven ou Gradle. Faz **layering automático** (separa dependências de classes — só a camada que mudou é reconstruída) e gera builds **reprodutíveis** (mesmo conteúdo → exatamente a mesma imagem). Empurra a imagem direto pro registry. Contraste com [[03-Dominios/Tecnologia/Java/Cloud-native e produção/06 - Buildpacks — imagem sem Dockerfile|Buildpacks]]: lá o lifecycle CNB precisa do daemon; aqui o plugin resolve tudo sozinho, daemonless.
