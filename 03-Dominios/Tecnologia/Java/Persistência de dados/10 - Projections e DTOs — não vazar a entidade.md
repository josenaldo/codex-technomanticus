...
Retornar `Order` em um `@RestController` acopla o contrato da API ao modelo de persistência. Renomear uma coluna, adicionar um campo `@Transient` ou alterar um relacionamento quebra a API sem aviso em tempo de compilação. O contrato da API é o DTO — veja [[03-Dominios/Tecnologia/Java/Web e APIs REST/05 - Serialização JSON com Jackson|Serialização JSON (Galho 9)]].
...
- [[03-Dominios/Tecnologia/Java/Dicionário de Java#projection (JPA)|projection (JPA)]]
