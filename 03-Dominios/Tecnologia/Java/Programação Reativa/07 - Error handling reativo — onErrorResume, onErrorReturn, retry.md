...
> No **Spring MVC** (imperativo), o tratamento de exceções da camada web é centralizado num `@ControllerAdvice` com métodos `@ExceptionHandler` — veja [[03-Dominios/Tecnologia/Java/Web e APIs REST/01 - O que é Spring MVC — a camada web sobre o container|O que é Spring MVC]] e o galho de tratamento de exceções imperativo. No **WebFlux** (reativo), o erro vive na própria cadeia (`onError*`) ou é tratado por um `WebExceptionHandler` global. São modelos distintos: aqui tratamos o lado reativo; não re-explicamos o handling imperativo.
