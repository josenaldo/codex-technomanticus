...
A distinção `just` vs `fromCallable`/`defer` é a fronteira entre **eager no assembly** e **lazy no subscribe** — o tema central da [[03-Dominios/Tecnologia/Java/Programação Reativa/04 - Nada acontece até o subscribe — lazy, assembly vs subscription, cold vs hot|próxima nota]].
