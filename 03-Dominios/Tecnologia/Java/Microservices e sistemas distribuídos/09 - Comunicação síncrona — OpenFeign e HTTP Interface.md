...
2. **HTTP Interface clients** — recurso **nativo do Spring Framework** (6.1+), com `@HttpExchange` e os verbos `@GetExchange`, `@PostExchange` etc. **Não depende de Spring Cloud**: roda sobre o `RestClient` ou o `WebClient` que você já conhece do [[03-Dominios/Tecnologia/Java/Web e APIs REST/15 - Clientes HTTP — RestClient, WebClient, RestTemplate|Galho 9]].
...
> O **`RestClient`/`WebClient`/`RestTemplate`** são o cliente HTTP *base* e foram explicados no [[03-Dominios/Tecnologia/Java/Web e APIs REST/15 - Clientes HTTP — RestClient, WebClient, RestTemplate|Galho 9]]. **Esta nota não os re-explica** — só mostra que o `@HttpExchange` é a camada *declarativa* por cima deles. Se a dúvida for "como configuro o `RestClient`?", a resposta está lá, não aqui.
...
- [[03-Dominios/Tecnologia/Java/Microservices e sistemas distribuídos/05 - Comunicação inter-serviços — síncrono vs assíncrono|Comunicação inter-serviços]] — o eixo síncrono vs assíncrono que esta nota aprofunda do lado síncrono
- [[03-Dominios/Tecnologia/Java/Microservices e sistemas distribuídos/16 - Resiliência IV — compondo os padrões|Resiliência (compondo os padrões)]] — por que toda chamada síncrona precisa de rede de proteção
- [[03-Dominios/Tecnologia/Java/Web e APIs REST/15 - Clientes HTTP — RestClient, WebClient, RestTemplate|Clientes HTTP (Galho 9)]] — fronteira: o cliente HTTP base sobre o qual o `@HttpExchange` roda (não re-explicado aqui)
