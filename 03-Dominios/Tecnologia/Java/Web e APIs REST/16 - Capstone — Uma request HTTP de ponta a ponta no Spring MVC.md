...
E há um ganho conceitual transversal: o Spring MVC é uma **implementação opinativa** sobre as specs Jakarta (Servlet, Bean Validation). Entender o pipeline também te ensina o que é Spring e o que é a plataforma portável por baixo — o tema da nota [[03-Dominios/Tecnologia/Java/Jakarta EE/14 - Jakarta EE hoje — a plataforma sob o Spring|Jakarta EE hoje]].
...
| --- | --- | --- |
| `DispatcherServlet` | **Servlet API** (`jakarta.servlet.http.HttpServlet`) | É, literalmente, *um* `Servlet` registrado no container. Todo o pipeline roda dentro de `service()`. Veja [[03-Dominios/Tecnologia/Java/Jakarta EE/03 - Servlet API — o alicerce HTTP\|Servlet API]]. |
| `@RestController` + `@RequestMapping` | **JAX-RS** (`@Path`, `@GET`, `@Produces`) | Roteamento declarativo de HTTP para métodos. O Spring usa convenção própria; JAX-RS é a spec portável. Veja [[03-Dominios/Tecnologia/Java/Jakarta EE/07 - JAX-RS — REST declarativo\|JAX-RS]]. |
| `@Valid` no `@RequestBody` | **Bean Validation** (`jakarta.validation`, `@NotNull`, `@Min`) | A *especificação* de validação é Jakarta; o Spring só decide *onde* e *quando* dispará-la no pipeline. Veja [[03-Dominios/Tecnologia/Java/Jakarta EE/08 - Bean Validation\|Bean Validation]]. |
| `HandlerInterceptor` | **Servlet Filter** (`jakarta.servlet.Filter`) | Ambos interceptam, mas em alturas diferentes: o `Filter` envolve o servlet inteiro (byte cru, agnóstico de handler); o `HandlerInterceptor` roda dentro do MVC, já sabendo qual controller vai atender. Veja [[03-Dominios/Tecnologia/Java/Web e APIs REST/11 - Interceptors vs Filters\|Interceptors vs Filters]]. |

A leitura: o que você escreve com anotações Spring tem um equivalente conceitual na plataforma. Se um dia você migrar para Quarkus ou Helidon (que falam JAX-RS direto), são os *mesmos conceitos* com outra fachada. O container por baixo do Spring continua sendo o mundo Jakarta — tema de [[03-Dominios/Tecnologia/Java/Spring Core e Boot/index|Spring Core e Boot]].
...
- [ ] **Status codes corretos.** 201 + `Location` ao criar, 200/204 ao atualizar, 404 vs 422 vs 400 distintos. O status é parte do contrato, não enfeite. → [[03-Dominios/Tecnologia/Java/Web e APIs REST/04 - ResponseEntity e status codes|ResponseEntity e status codes]]
- [ ] **DTO de borda, nunca a entidade.** Exponha `OrderResponse`, não `Order` (a entidade JPA). Desacopla o modelo persistente do contrato HTTP e evita vazar campos internos / lazy-loading na serialização. → [[03-Dominios/Tecnologia/Java/Web e APIs REST/05 - Serialização JSON com Jackson|Serialização JSON com Jackson]]
- [ ] **Validação na borda** com `@Valid` no `@RequestBody`, falhando rápido com 400 antes de tocar a regra de negócio. → [[03-Dominios/Tecnologia/Java/Web e APIs REST/08 - Validação na borda|Validação na borda]]
- [ ] **Erro padronizado (RFC 9457).** Um `@RestControllerAdvice` central que emite `ProblemDetail`/`application/problem+json` para todo erro previsível. → [[03-Dominios/Tecnologia/Java/Web e APIs REST/10 - Problem Details — RFC 9457|Problem Details — RFC 9457]]
- [ ] **OpenAPI / Swagger** gerado e versionado, para que o contrato seja descobrível e testável por consumidores. → [[03-Dominios/Tecnologia/Java/Web e APIs REST/12 - Documentando a API com OpenAPI e Swagger|Documentando a API com OpenAPI e Swagger]]
...
**O fix.** Trate status como parte do design do endpoint. Use `ResponseEntity.created(uri)`, `.noContent()`, `.ok()` deliberadamente — veja [[03-Dominios/Tecnologia/Java/Web e APIs REST/04 - ResponseEntity e status codes|ResponseEntity e status codes]].
...
**O fix.** `@Valid` na borda para o que é estrutural; regras de negócio no domínio lançando exceções tratadas pelo advice. Os dois caminhos convergem no mesmo `ProblemDetail`. Veja [[03-Dominios/Tecnologia/Java/Web e APIs REST/08 - Validação na borda|Validação na borda]].
...
| --- | --- |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/01 - O que é Spring MVC — a camada web sobre o container|01 — O que é Spring MVC]] | "O que está rodando por baixo do meu controller?" |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/06 - O pipeline do DispatcherServlet|06 — Pipeline do DispatcherServlet]] | "Quem chama meu método e em que ordem?" |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/03 - Recebendo dados da request|03 — Recebendo dados da request]] | "Como params/body viram parâmetros do método?" |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/04 - ResponseEntity e status codes|04 — ResponseEntity e status codes]] | "Como controlo status, headers e corpo da resposta?" |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/07 - Content negotiation|07 — Content negotiation]] | "Como o Spring decide JSON vs XML?" |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/08 - Validação na borda|08 — Validação na borda]] | "Onde rejeito entrada inválida?" |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/09 - Tratamento de exceções com @ControllerAdvice|09 — @ControllerAdvice]] | "Onde centralizo o tratamento de erros?" |
| [[03-Dominios/Tecnologia/Java/Web e APIs REST/10 - Problem Details — RFC 9457|10 — Problem Details]] | "Qual o formato padrão de erro HTTP?" |
...
- [[03-Dominios/Tecnologia/Java/Web e APIs REST/10 - Problem Details — RFC 9457|Problem Details — RFC 9457]]
- [[03-Dominios/Tecnologia/Java/Web e APIs REST/08 - Validação na borda|Validação na borda]]
- [[03-Dominios/Tecnologia/Java/Jakarta EE/07 - JAX-RS — REST declarativo|JAX-RS]] (a spec — o outro caminho)
- [[03-Dominios/Tecnologia/Java/Spring Core e Boot/index|Spring Core e Boot]] (o container)
