...
![[evolucao-eng-graph-engineering.png]]
*Carlos E. Perez (@IntuitMachine) — os quatro tipos de aresta: PAIR (OPTIMIZE ↔ WATCH, pega a trapaça), HIERARCHY (SLOW define o alvo do FAST), ARBITRATE (nó de trade-off entre SPEED e DEPTH), AUDIT (um loop que vigia o vigia).*
...
- **Perez, C. E. (@IntuitMachine)** — thread e material visual sobre os quatro tipos de aresta (PAIR, HIERARCHY, ARBITRATE, AUDIT), a composição champion-challenger + drift monitor + rollback + held-out eval, e as frases "a unidade de design não é mais o loop, é a rede de loops" e "a confiabilidade mora nas arestas, não nos nós", citado na pesquisa consolidada deste galho. Fonte da imagem embutida nesta nota.
