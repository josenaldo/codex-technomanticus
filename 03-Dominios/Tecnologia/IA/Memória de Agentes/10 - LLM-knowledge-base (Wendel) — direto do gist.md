...
> [!abstract] TL;DR
> `LLM-knowledge-base` é a implementação Python de referência do **LLM Wiki Pattern** do Karpathy, mantida por Wendel em `https://github.com/wendeus0/LLM-knowledge-base`. O mecanismo é um ciclo de quatro etapas — *ingest → compile → Q&A/search → heal/lint* — em que documentos brutos (EPUB, PDF com OCR opcional, web) viram uma wiki markdown compilada por LLM, consultável via busca híbrida (keyword + BM25 + RRF) e mantida por healing/lint automáticos, com um subsistema de claims lifecycle rastreando confiança, supersessão e decaimento de cada afirmação. Tecnicamente é notável por manter a separação engine/dados via `KB_DATA_DIR`, integrar com Obsidian através do plugin `obsidian-terminal` e sustentar 311 testes com 90%+ de cobertura e 16 ADRs documentando as decisões de design. Vale como **referência canônica em código real** para estudar como o pattern do Karpathy vira software — não como SaaS pronto: é CLI de autor único sob AGPL-3.0, sem SLA, sem suporte e sem UX polida.
