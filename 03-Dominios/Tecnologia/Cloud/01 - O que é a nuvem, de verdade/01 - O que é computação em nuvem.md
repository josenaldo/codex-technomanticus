...
> "Um modelo para permitir acesso conveniente e sob demanda, via rede, a um conjunto compartilhado de recursos computacionais configuráveis (...) que podem ser rapidamente provisionados e liberados com esforço mínimo de gerenciamento ou interação com o provedor de serviço."
...
**O MVP que não pode esperar hardware.** Uma startup de dois desenvolvedores decide validar uma ideia. No mundo pré-nuvem, mesmo o MVP mais simples exigiria alguém indo atrás de um servidor — comprar, alugar num datacenter com contrato mínimo, ou usar hosting compartilhado com limitações severas de configuração. Hoje, os dois abrem conta num provedor, sobem uma instância pequena, apontam o domínio, e o produto está no ar em uma tarde — pagando centavos por hora enquanto ninguém usa, e escalando (manual ou automaticamente) se o produto pegar tração. O capital que antes seria imobilizado em hardware fica livre para outra coisa. Essa é a promessa mais citada da nuvem para startups, e ela é real precisamente porque as cinco características do NIST eliminam o gargalo de aprovação e aquisição.

**O pico sazonal que só dura um dia.** Volte à Black Friday do início desta nota, agora resolvida com nuvem: em vez de comprar 20 servidores em outubro para usar um dia em novembro, o time configura a infraestrutura para escalar automaticamente conforme o tráfego sobe — mais instâncias entrando quando a fila de requisições cresce, saindo quando ela esvazia. No dia seguinte, a capacidade volta ao normal sozinha, e a fatura reflete exatamente as horas de pico, não um ano inteiro de capacidade ociosa. É a mesma dor do cenário de abertura, resolvida pela combinação de elasticidade rápida com serviço medido.

**O job pesado que roda três horas por mês.** Um time de dados precisa rodar um processamento pesado — reindexar um catálogo, retreinar um modelo, reprocessar um histórico — que consome muita CPU e memória, mas só uma vez por mês, por poucas horas. Manter hardware dedicado para essa carga o resto do tempo seria puro desperdício. Na nuvem, o time sobe uma máquina grande sob demanda, roda o job, desliga, e paga só pelas três horas. É o mesmo padrão de sempre — self-service, elasticidade, medição — aplicado a uma carga que é intermitente por natureza, não só sazonal.
