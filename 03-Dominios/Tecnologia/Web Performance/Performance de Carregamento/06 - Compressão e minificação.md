...
> **O que acontece:** o time habilita compressão gzip/Brotli para *tudo*, incluindo imagens, vídeos e fontes `woff2`, e ganha ~0% — às vezes os arquivos até incham um pouco.
> **Por quê:** JPEG, PNG, AVIF, WebP, MP4 e `woff2` **já são formatos comprimidos**. Passar um compressor de propósito geral por cima não acha mais redundância; só gasta CPU.
> **Como evitar:** comprima apenas recursos de **texto** (HTML, CSS, JS, SVG, JSON, `.ico` em alguns casos). Deixe os formatos binários já otimizados em paz — a otimização deles é a da [[03-Dominios/Tecnologia/Web Performance/Performance de Carregamento/04 - Otimização de imagens|nota 04]].
