...
Esta é a nota de fechamento do galho [[03-Dominios/Fundamentos/Banco de Dados/index|Banco de Dados]]. As quinze notas anteriores ensinaram o conteúdo; esta ensina a **performar** o conteúdo numa sala de entrevista. São coisas diferentes. Saber o que é um índice composto é uma; saber dizer, sob pressão, em inglês, por que você criaria `(paciente_id, data DESC)` e não `(data, paciente_id)` — defendendo a escolha com `EXPLAIN` — é outra.
