...
A solução é a **pilha de execução** (*call stack*): uma região de memória que cresce para baixo (endereços decrescentes) gerenciada pelo registrador `rsp` (stack pointer). É literalmente a estrutura de dados pilha que você conhece de [[03-Dominios/Fundamentos/Estruturas de Dados/04 - Pilhas, filas e deques]] — LIFO, push e pop — só que implementada no hardware.
