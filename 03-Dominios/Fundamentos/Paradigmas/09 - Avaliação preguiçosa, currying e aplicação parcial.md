...
Em Java, um `Stream` é uma **esteira preguiçosa**. As operações *intermediárias* (`map`, `filter`, `limit`) não executam nada — só montam o pipeline. Só a operação *terminal* (`collect`, `forEach`, `findFirst`) puxa o gatilho e faz os dados fluírem. Veja `[[03-Dominios/Java/Collections e Streams/index|Streams (Java)]]`.
...
- [[03-Dominios/Java/Collections e Streams/index|Streams (Java)]] — laziness opt-in na JVM
