...
> Causa relatada pelo executor da nota 15: leu o plano como **lista fechada de ações** e não expandiu
> além dela. **Correção de instrução:** as ações listadas são o mínimo; fechar o piso com conteúdo
> técnico novo e sourced é parte do trabalho, não extra. A proibição de padding continua valendo —
> déficit residual honesto ainda é melhor que enchimento.
...
- **Enriquecimento:** ✅ feita (2026-08-01, passe 2)
...
- **Resultado:** 397 linhas (era 270). **Piso 400 NÃO atingido — faltam 3 linhas, déficit declarado em vez de preenchido com enchimento** (comportamento correto sob a regra anti-padding). E1 TL;DR para 3 linhas · E4 `## Casos práticos` formaliza o caso real `@Mock`→`InMemoryUserRepository`; segundo cenário não fabricado, gap consciente em callout · E5 ponte para Testes JS/07 e Arqueologia/11 (os dois paths que o diagnóstico errara, agora corretos) · E7 tabela PT↔EN com 16 termos · E8 os 2 callouts movidos + 1 derivada ("mockar por preguiça") · L2 `## Fontes` com Fowler/Dodds/Khorikov · M1 PyCon Greece, Kalaitzis (`QQUwZQvuFCQ`, legenda baixada e ID reconferido). Conteúdo novo: os quatro pilares de Khorikov, diagrama de decisão fake×mock×stub e exemplos Java genéricos. **Passe 2 (2026-08-01): 397 → 409, piso atingido.** Bloco novo `### O critério prático: para que serve essa API?` — critério de Khorikov (é comportamento observável se um cliente usa a API para atingir um objetivo) e o sintoma concreto de precisar tornar método `private` em `public`/`@VisibleForTesting` só para o teste, com a saída de extrair classe em vez de expor. Fonte nova verificada via WebFetch.
...
- **Resultado:** 364 linhas (era 330). **Piso 400 NÃO atingido — déficit residual de 36 linhas, declarado em vez de preenchido.** Justificativa do executor: a nota é prosa longa (parágrafo denso conta 1 linha), e fechar exigiria inflar texto ou inventar sub-tópico fora do plano — ambos vedados. E1 TL;DR para 3 linhas · E4 `## Casos práticos` formaliza os DOIS casos reais (comissão de 5 condições · cadastro de 30 campos) lado a lado, sem alterar fato algum; as menções originais viraram pontes · E5/L1 ponte para Python/Testes/08 e Arqueologia/14 · E7 vocabulário virou tabela · E8 os 3 `[!warning]` movidos · L2 `## Fontes` com as 7 URLs + Ian Cooper e Fowler · M1 Ian Cooper, "TDD, Where Did It All Go Wrong" na DevTernity (`EZ05e7EMOLM`, 12.600 linhas de VTT baixadas, ID reconferido). **Passe 2 (2026-08-01): 364 → 392. Déficit residual de 8 linhas, declarado.** Bloco novo `## Test desiderata: as propriedades em tensão` — as 12 propriedades de Kent Beck em tabela, amarradas à tensão classicista/mockista, ao loop externo/interno e ao teste de caracterização em legado. O executor preferiu um bloco coeso a espalhar frases pela nota; o resto exigiria repetir a nota 08 ou tocar nos casos reais, ambos vedados.
...
- **Resultado:** 297 linhas (era 239). **Piso 400 NÃO atingido — déficit residual de ~103 linhas, declarado em vez de preenchido** (a maior lacuna do galho até aqui). E1 TL;DR para 3 linhas · E4 `## Casos práticos` formaliza o caso real do Awaitility com os fatos preservados literalmente, registrando que só há 1 cenário · E5 ponte para Testes JS/16 · E8 os 3 callouts movidos e individualizados · L2 `## Fontes` com Fowler, Google Testing Blog e Awaitility clicáveis · M1 Andrei Solntsev no Jfokus, 50 min (`18J2_4a4Cl4`, legenda baixada e ID reconferido). Conteúdo novo: seção sobre retry automatizado como paliativo (Gradle Test Retry Plugin e Surefire `rerunFailingTestsCount`, com fonte oficial) e diagrama da semântica de `failOnPassedAfterRetry`. **Passe 2 (2026-08-01): 297 → 400, piso atingido, déficit zerado.** Blocos novos sourced: taxonomia empírica de Luo et al. (FSE 2014) com proporções (async wait 45% · concorrência 20% · ordem 12%) — **números conferidos contra o paper** — e árvore de diagnóstico em Mermaid · isolamento de recursos sob paralelismo e casos de calendário (DST, bissexto, viradas) · `victim`/`polluter`/`cleaner` com vocabulário do iDFlakies (ICST 2019) e diagrama de sequência · detecção por cobertura diferencial (DeFlaker, ICSE 2018) · quando deletar o teste é a decisão certa (Fowler). Caso do Awaitility intocado.
...
- **Resultado:** 362 linhas (era 322). **Piso 500 NÃO atingido — déficit residual de 138 linhas, declarado.** E1 TL;DR para 5 linhas, uma por técnica · E5 ponte para Testes JS/11, Acessibilidade/14, Arqueologia/11 e Java/Testes/20 · E7 tabela PT↔EN · E8 resolvido: era a nota mais pobre do galho (1 callout); o `[!danger] Snapshot fatigue` foi movido e 3 armadilhas derivadas do próprio texto (propriedade fraca, contract em monolito/terceiro, smoke inflado) · L2 `## Fontes` com as 5 URLs · M1 John Hughes, criador do QuickCheck, "Don't Write Tests" no Curry On! (`hXnS_Xjwk2Y`, legenda de 7.096 linhas baixada e ID reconferido). **E4 = buraco declarado.** O executor avaliou não haver mais lacuna técnica sourced sem inflar. **Passe 2 (2026-08-01): 362 → 502 linhas, piso Magus atingido, déficit zerado.** Aprofundou o mecanismo das quatro técnicas em vez de listar novas: property-based com geradores/`Arbitrary`, as quatro categorias clássicas de propriedade (invariante, idempotência, metamórfica, oráculo) com exemplo distinto cada, e stateful/model-based testing com máquina de estados completa (docs do Hypothesis) · snapshot com critério de bom/mau candidato, normalização de campos voláteis por property matchers (docs do Jest) e a distinção explícita frente a approval testing, linkando Arqueologia/11 · contract consumer-driven × provider-driven (Pact × Spring Cloud Contract), fluxo do broker `can-i-deploy`/`record-deployment` e provider states · smoke × health check × synthetic monitoring, linkando Operação/4-01 (path verificado) em vez de duplicar. 5 URLs novas verificadas antes de citar.
