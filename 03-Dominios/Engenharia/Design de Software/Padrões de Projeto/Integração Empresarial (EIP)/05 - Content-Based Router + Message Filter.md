...
> São primos com uma diferença de **onde** mora a decisão. O **Message Filter** é um passo **no fluxo** (um filtro no pipeline) que descarta antes de seguir. O **Selective Consumer** ([[10 - Consumers - Polling × Event-Driven]]) é o **consumidor** que só retira do canal as mensagens que casam com um critério (o broker filtra na entrega — ex. *message selector* do JMS). E o `filter()` de Kafka Streams/RxJS é a mesma ideia sobre streams in-process. Mesma intenção (deixar passar só o relevante); o que muda é se o descarte acontece no pipeline, no broker, ou no operador de stream.
