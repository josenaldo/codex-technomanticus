...
> Aqui tratamos da **integração que valida** o JWT. A anatomia do token em si — header, payload, assinatura, como ele é assinado e verificado — está em [[03-Dominios/Java/Segurança/08 - JWT — estrutura, assinatura e validação|JWT]]. Não vamos re-explicar isso.
...
**Fix:** numa API stateless autenticada por Bearer token, desligue CSRF — `.csrf(c -> c.disable())`. A defesa CSRF protege fluxos baseados em cookie de sessão, que aqui não existem. (Detalhes em [[03-Dominios/Java/Segurança/10 - CSRF — por que ligado por default e quando desligar|CSRF]].)
...
- [[03-Dominios/Java/Dicionário de Java]] (verbetes OAuth2 Resource Server / JWKS / JwtAuthenticationConverter)
