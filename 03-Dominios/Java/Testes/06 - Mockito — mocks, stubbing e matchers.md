...
3. **Verificação** — você confere que o mock foi (ou não foi) chamado como esperado (assunto da [[03-Dominios/Java/Testes/07 - Mockito — verify, ArgumentCaptor e quando NÃO mockar|próxima nota]]).
...
O `thenAnswer` brilha quando o retorno depende da entrada: aqui ele copia o e-mail e o total recebidos e devolve um `Order` com id gerado, imitando o comportamento de um `save` real sem tocar no banco. Note o uso de [[03-Dominios/Java/Testes/03 - AssertJ — fluent assertions|AssertJ]] para as asserções.
