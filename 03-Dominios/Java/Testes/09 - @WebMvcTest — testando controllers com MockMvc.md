...
> O *como* o `DispatcherServlet` resolve handlers, despacha exceções e serializa o corpo está descrito em [[03-Dominios/Java/Web e APIs REST/06 - O pipeline do DispatcherServlet|O pipeline do DispatcherServlet]]. Aqui o foco é **testar** esse pipeline, não reexplicá-lo.
...
> O `@RestController` e seus mapeamentos (`@GetMapping`, `@PostMapping`, `@PathVariable`, `@RequestBody @Valid`) são do Galho 9 — veja [[03-Dominios/Java/Web e APIs REST/02 - @RestController e os mapeamentos|@RestController e os mapeamentos]]. O `@WebMvcTest` apenas **exercita** esses mapeamentos.
...
**Fix:** quando o objetivo for testar a stack **real** com servidor de verdade, use `@SpringBootTest(webEnvironment = RANDOM_PORT)` com `WebTestClient`/`TestRestTemplate` — veja [[03-Dominios/Java/Testes/12 - Testes de integração ponta a ponta|Testes de integração ponta a ponta]]. `@WebMvcTest` + `MockMvc` é deliberadamente **in-memory**.
...
- [[03-Dominios/Java/Testes/13 - Testando a segurança|Testando a segurança]]
