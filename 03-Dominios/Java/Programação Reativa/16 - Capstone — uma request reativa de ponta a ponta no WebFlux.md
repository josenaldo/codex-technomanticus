...
**Fix.** Escolha por problema, não por hype. WebFlux pra **streaming real e backpressure ponta a ponta** (onde `request(n)` é insubstituível); MVC + Virtual Threads pra **CRUD de alta concorrência**. O confronto detalhado está na nota 14 — [[03-Dominios/Java/Programação Reativa/14 - Reativo vs Virtual Threads — o confronto honesto|Reativo vs Virtual Threads]].
...
- [[03-Dominios/Java/Programação Reativa/13 - R2DBC — persistência reativa sem EntityManager|R2DBC]]
