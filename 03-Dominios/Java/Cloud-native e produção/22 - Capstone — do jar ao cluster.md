...
3. **Enxugar e escanear.** Base distroless ou enxuta, usuário não-root, scan de CVE no pipeline — ver [[03-Dominios/Java/Cloud-native e produção/05 - Imagem enxuta e segura — distroless e scanning|Imagem enxuta e segura]].
4. **A JVM dentro do container.** A JVM moderna é *container-aware*: lê os limites do cgroup e dimensiona heap por `MaxRAMPercentage`. Sem isso, ela acha que tem a RAM da máquina inteira e o kernel mata o pod. Ver [[03-Dominios/Java/Cloud-native e produção/02 - A JVM dentro de um container|A JVM dentro de um container]].
5. **Probes.** O Actuator expõe `/actuator/health/liveness` e `/actuator/health/readiness` como contratos distintos: *liveness* = "estou vivo, não me reinicie"; *readiness* = "posso receber tráfego agora?". Ver [[03-Dominios/Java/Cloud-native e produção/10 - Health e probes — o contrato com o orquestrador|Health e probes]].
6. **Config e recursos.** Config vem de fora (ConfigMap/Secret → variáveis de ambiente), e `requests`/`limits` declaram o apetite do pod. Ver [[03-Dominios/Java/Cloud-native e produção/11 - Config e recursos no Kubernetes|Config e recursos no Kubernetes]].
7. **Graceful shutdown.** No SIGTERM, o app marca readiness como *down*, drena conexões em voo e morre dentro do timeout — casado com o `preStop` e `terminationGracePeriodSeconds` do pod. Ver [[03-Dominios/Java/Cloud-native e produção/12 - Graceful shutdown e deploy sem downtime|Graceful shutdown e deploy sem downtime]].
8. **Observabilidade.** Os três *seams*: **métricas** via Micrometer → Prometheus; **traces** via OpenTelemetry → Collector; **logs** estruturados (JSON) no stdout. Panorama em [[03-Dominios/Java/Cloud-native e produção/13 - Observabilidade de operação — o panorama e os 3 seams|Observabilidade — o panorama]].
9. **CI/CD.** Um pipeline constrói, testa, escaneia, publica a imagem e aplica o manifesto. Ver [[03-Dominios/Java/Cloud-native e produção/20 - CI-CD e o caminho até produção|CI-CD e o caminho até produção]].
...
- **JVM ou native?** A JVM dá *peak throughput* e ferramental de diagnóstico maduro (JFR, async-profiler); o native dá *startup* quase instantâneo e *footprint* baixo, ao custo de build lento, reflexão fechada por hints e profiling diferente. A decisão honesta está em [[03-Dominios/Java/Cloud-native e produção/21 - Native vs JVM — a decisão honesta|Native vs JVM]] e o conceito em [[03-Dominios/Java/Cloud-native e produção/08 - GraalVM Native Image — conceito e trade-offs|GraalVM Native Image — conceito]].
- **Quanto trace coletar?** *Head sampling* decide cedo (barato, mas pode descartar o trace de um erro raro); *tail sampling* decide depois de ver o trace inteiro no Collector (caro, mas pega os outliers). Ver [[03-Dominios/Java/Cloud-native e produção/16 - OpenTelemetry Collector e sampling de produção|OpenTelemetry Collector e sampling]].
...
- **Ser observável sem `kubectl exec`.** Quando algo trava às 3h da manhã, você precisa de métricas, traces e logs já fluindo — não de abrir um shell no pod. Inclui **profiling sob carga** ([[03-Dominios/Java/Cloud-native e produção/18 - Profiling e diagnóstico sob carga — produção|Profiling e diagnóstico sob carga]]) e **continuous profiling** ([[03-Dominios/Java/Cloud-native e produção/19 - Continuous profiling no cluster — Pyroscope e async-profiler|Continuous profiling no cluster]]).
...
| Detalhe | [[03-Dominios/Java/Cloud-native e produção/04 - Dockerfile na prática — multi-stage e layered jar\|nota 04]] | [[03-Dominios/Java/Cloud-native e produção/06 - Buildpacks — imagem sem Dockerfile\|nota 06]] | [[03-Dominios/Java/Cloud-native e produção/07 - Jib — imagem daemonless\|nota 07]] |
...
| Detalhe | [[03-Dominios/Java/Cloud-native e produção/21 - Native vs JVM — a decisão honesta\|nota 21]] | [[03-Dominios/Java/Cloud-native e produção/09 - Native Image com Spring — Spring AOT na prática\|nota 09]] |
...
| Detalhe | [[03-Dominios/Java/Cloud-native e produção/16 - OpenTelemetry Collector e sampling de produção\|nota 16]] | idem |
...
|---|---|
| "O que significa estar pronto pra produção?" | [[03-Dominios/Java/Cloud-native e produção/01 - Production-ready e cloud-native — a tese honesta\|01 - Production-ready e cloud-native — a tese honesta]] |
| "O pod é morto por OOM mesmo com pouca carga" | [[03-Dominios/Java/Cloud-native e produção/02 - A JVM dentro de um container\|02 - A JVM dentro de um container]] |
| "Qual é o panorama de empacotar em imagem?" | [[03-Dominios/Java/Cloud-native e produção/03 - Empacotando o app numa imagem — o panorama\|03 - Empacotando o app numa imagem — o panorama]] |
| "Como escrevo um Dockerfile eficiente?" | [[03-Dominios/Java/Cloud-native e produção/04 - Dockerfile na prática — multi-stage e layered jar\|04 - Dockerfile na prática — multi-stage e layered jar]] |
| "Minha imagem é gorda e cheia de CVE" | [[03-Dominios/Java/Cloud-native e produção/05 - Imagem enxuta e segura — distroless e scanning\|05 - Imagem enxuta e segura — distroless e scanning]] |
| "Quero imagem sem manter Dockerfile" | [[03-Dominios/Java/Cloud-native e produção/06 - Buildpacks — imagem sem Dockerfile\|06 - Buildpacks — imagem sem Dockerfile]] |
| "Meu CI não tem Docker daemon" | [[03-Dominios/Java/Cloud-native e produção/07 - Jib — imagem daemonless\|07 - Jib — imagem daemonless]] |
| "O que é native image e quais os trade-offs?" | [[03-Dominios/Java/Cloud-native e produção/08 - GraalVM Native Image — conceito e trade-offs\|08 - GraalVM Native Image — conceito e trade-offs]] |
| "Como faço Spring rodar como native?" | [[03-Dominios/Java/Cloud-native e produção/09 - Native Image com Spring — Spring AOT na prática\|09 - Native Image com Spring — Spring AOT na prática]] |
| "O cluster reinicia meu pod sem motivo" | [[03-Dominios/Java/Cloud-native e produção/10 - Health e probes — o contrato com o orquestrador\|10 - Health e probes — o contrato com o orquestrador]] |
| "Como injeto config e dimensiono recursos?" | [[03-Dominios/Java/Cloud-native e produção/11 - Config e recursos no Kubernetes\|11 - Config e recursos no Kubernetes]] |
| "O deploy corta requisições no meio" | [[03-Dominios/Java/Cloud-native e produção/12 - Graceful shutdown e deploy sem downtime\|12 - Graceful shutdown e deploy sem downtime]] |
| "Por onde começo a observar o serviço?" | [[03-Dominios/Java/Cloud-native e produção/13 - Observabilidade de operação — o panorama e os 3 seams\|13 - Observabilidade de operação — o panorama e os 3 seams]] |
| "Como exponho métricas pro Prometheus?" | [[03-Dominios/Java/Cloud-native e produção/14 - Métricas em produção — Micrometer e Prometheus\|14 - Métricas em produção — Micrometer e Prometheus]] |
| "Preciso de dashboards e alertas" | [[03-Dominios/Java/Cloud-native e produção/15 - Dashboards e alertas — Grafana\|15 - Dashboards e alertas — Grafana]] |
| "Quanto trace coletar sem estourar custo?" | [[03-Dominios/Java/Cloud-native e produção/16 - OpenTelemetry Collector e sampling de produção\|16 - OpenTelemetry Collector e sampling de produção]] |
| "Meus logs são texto solto, sem correlação" | [[03-Dominios/Java/Cloud-native e produção/17 - Logs estruturados em produção\|17 - Logs estruturados em produção]] |
| "A latência subiu sob carga — onde está o gargalo?" | [[03-Dominios/Java/Cloud-native e produção/18 - Profiling e diagnóstico sob carga — produção\|18 - Profiling e diagnóstico sob carga — produção]] |
| "Quero perfilar continuamente no cluster" | [[03-Dominios/Java/Cloud-native e produção/19 - Continuous profiling no cluster — Pyroscope e async-profiler\|19 - Continuous profiling no cluster — Pyroscope e async-profiler]] |
| "Como automatizo o caminho até produção?" | [[03-Dominios/Java/Cloud-native e produção/20 - CI-CD e o caminho até produção\|20 - CI-CD e o caminho até produção]] |
| "JVM ou native — qual escolho de verdade?" | [[03-Dominios/Java/Cloud-native e produção/21 - Native vs JVM — a decisão honesta\|21 - Native vs JVM — a decisão honesta]] |
| "Quero ver o fluxo inteiro montado" | [[03-Dominios/Java/Cloud-native e produção/22 - Capstone — do jar ao cluster\|22 - Capstone — do jar ao cluster]] (você está aqui) |
...
- [[03-Dominios/Java/Cloud-native e produção/04 - Dockerfile na prática — multi-stage e layered jar|Dockerfile na prática (empacotamento)]]
- [[03-Dominios/Java/Cloud-native e produção/10 - Health e probes — o contrato com o orquestrador|Health e probes (contrato com o orquestrador)]]
- [[03-Dominios/Java/Cloud-native e produção/13 - Observabilidade de operação — o panorama e os 3 seams|Observabilidade — os 3 seams]]
- [[03-Dominios/Java/Cloud-native e produção/20 - CI-CD e o caminho até produção|CI-CD e o caminho até produção]]
- [[03-Dominios/Java/Cloud-native e produção/21 - Native vs JVM — a decisão honesta|Native vs JVM (a decisão honesta)]]
