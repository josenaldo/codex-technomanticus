...
A **fronteira-assinatura é quádrupla** — quatro temas pertencem a este galho só de relance, porque foram explicados em profundidade em outros galhos (aqui se linka, não se re-explica): módulos/JPMS, jlink e GraalVM/AOT (em nível de JVM) estão no Galho 3 — a imagem nativa de produção (GraalVM native image) é o [[03-Dominios/Java/Cloud-native e produção/08 - GraalVM Native Image — conceito e trade-offs|Galho 17]]; jpackage está no Galho 6; starters e repackage estão no Galho 8; teoria de teste, cobertura e mutation testing estão no Galho 13.
