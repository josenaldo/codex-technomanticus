...
- **Passo 2 — o proxy não é decoração.** O `SimpleJpaRepository` por trás do proxy é quem realmente fala com o `EntityManager`. O método derivado ou a `Specification` são montados aqui. Veja [[03-Dominios/Java/Persistência de dados/04 - Spring Data repositories — JpaRepository e query methods derivados|Spring Data repositories]].
- **Passo 3 — `Pageable` gera duas queries.** Uma de dados (com `LIMIT`/`OFFSET`) e uma de `count` para o total. Isso é esperado e não é N+1 — é o preço da paginação. Veja [[03-Dominios/Java/Persistência de dados/11 - Paginação e ordenação — Pageable, Page e Slice|Paginação e ordenação]].
...
- **Passo 9 — a projeção é a defesa.** Converter para DTO **antes** de a entidade escapar do contexto evita tanto vazamento de domínio no JSON quanto `LazyInitializationException` na serialização. Veja [[03-Dominios/Java/Persistência de dados/10 - Projections e DTOs — não vazar a entidade|Projections e DTOs]].
...
- **O `@Transactional` é um proxy, não mágica.** É exatamente o mesmo mecanismo de [[03-Dominios/Java/Spring Core e Boot/09 - AOP e proxies no Spring|AOP e proxies]]: um advice envolve o método. Por isso a auto-invocação (chamar um método `@Transactional` de dentro da mesma classe) **não** abre transação — o proxy é contornado.
...
- **Rollback é por exceção de runtime.** Por padrão, só `RuntimeException`/`Error` disparam rollback. Checked exceptions **não** revertem a menos que declaradas em `rollbackFor`. Veja [[03-Dominios/Java/Persistência de dados/12 - Transações operacionais — @Transactional propagação, isolamento, rollback, readOnly|Transações operacionais]].
...
Para o lado Jakarta dessa tabela, veja [[03-Dominios/Java/Jakarta EE/09 - JPA — a especificação de persistência|JPA (a spec)]] e [[03-Dominios/Java/Jakarta EE/11 - JTA — transações na plataforma|JTA]].
...
- **Sempre LAZY por padrão.** Nunca `FetchType.EAGER` em coleções. EAGER é a porta de entrada do N+1 e tira de você o controle do fetch plan. Veja [[03-Dominios/Java/Persistência de dados/07 - Fetch strategies — LAZY, EAGER e a LazyInitializationException|Fetch strategies]].
...
| --- | --- |
| N+1 — uma query virou centenas | [[03-Dominios/Java/Persistência de dados/08 - O problema N+1 e suas soluções — @EntityGraph, JOIN FETCH, batch size|nota 08]] |
| `LazyInitializationException` na serialização | [[03-Dominios/Java/Persistência de dados/07 - Fetch strategies — LAZY, EAGER e a LazyInitializationException|nota 07]] |
| vazou entidade no JSON / acoplou domínio à API | [[03-Dominios/Java/Persistência de dados/10 - Projections e DTOs — não vazar a entidade|nota 10]] |
| checked exception não fez rollback | [[03-Dominios/Java/Persistência de dados/12 - Transações operacionais — @Transactional propagação, isolamento, rollback, readOnly|nota 12]] |
| lost update sob concorrência | [[03-Dominios/Java/Persistência de dados/13 - Locking — optimistic (@Version) e pessimistic|nota 13]] |
| JPA não dá conta do relatório analítico | [[03-Dominios/Java/Persistência de dados/15 - Consultas dinâmicas e os limites da JPA — Specifications, Criteria e SQL|nota 15]] |
...
- [[03-Dominios/Java/Persistência de dados/12 - Transações operacionais — @Transactional propagação, isolamento, rollback, readOnly|Transações operacionais]]
- [[03-Dominios/Java/Jakarta EE/09 - JPA — a especificação de persistência|JPA (a spec)]]
- [[03-Dominios/Java/Jakarta EE/11 - JTA — transações na plataforma|JTA]]
- [[03-Dominios/Java/Spring Core e Boot/09 - AOP e proxies no Spring|AOP e proxies]]
