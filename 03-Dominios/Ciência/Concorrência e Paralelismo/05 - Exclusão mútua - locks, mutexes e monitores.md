...
O monitor é a fundação direta do `synchronized`/`wait`/`notify` de Java e dos `Lock`/`Condition` modernos. A JVM usa semântica de tipo Mesa — daí o `while` obrigatório. A mecânica concreta dessas APIs mora em [[03-Dominios/Tecnologia/Java/Concorrência e paralelismo/index|Concorrência (Java)]]; aqui fica o conceito.
...
- [[03-Dominios/Tecnologia/Java/Concorrência e paralelismo/index|Concorrência (Java)]] — `ReentrantLock`, `synchronized`, `StampedLock` na prática
