...
- **Grafos como estrutura de dados e algoritmos (BFS/DFS/Dijkstra)** → [[03-Dominios/Ciência/Estruturas de Dados/11 - Grafos - travessia e algoritmos|Estruturas de Dados]]. Aqui (notas 16–18) é o **lado matemático**: definições, Euler/Hamilton, planaridade, coloração, matching, árvore como objeto.
- **Diagonalização e o incomputável** → [[03-Dominios/Ciência/Teoria da Computação/10 - Decidível, reconhecível e a máquina universal|Teoria da Computação]]. Aqui (nota 13) mora a **cardinalidade** (contável × incontável) e a diagonalização de Cantor que aquela teoria *usa*.
