---
title: "Seis currículos, uma carreira"
created: 2026-08-20
updated: 2026-08-20
type: concept
status: seedling
...
aliases:
  ...
  - Capstone do galho Currículo
  - Currículo de estagiário a staff
  - Seis níveis, um currículo cada
  - Exemplo completo de currículo por nível
---
...
A quinta peça deixa de ser reconstrução e passa a ser registro direto: o documento abaixo reúne, sem inventar nem uma linha, o conteúdo do autor deste vault reunido de três fontes de naturezas diferentes — a página inicial e a página de experiências (`/experiences`), ambas verificadas ao vivo em 2026-08-20, e o conteúdo de contratação, retirado do arquivo `content/pages/pt/hiring.md` do repositório do site (o site está em reformulação e essa página específica não está publicada no domínio ao vivo no momento desta escrita). Ele não é a cópia de um único arquivo de currículo em PDF — é uma montagem, transparente sobre a própria origem, de três peças que já são, cada uma, uma das quatro saídas da mesma âncora que a [[03-Dominios/Carreira/Currículo/20 - A âncora|nota 20]] descreveu: a página inicial fala com quem está decidindo contratar; o conteúdo de contratação fala com quem está avaliando o encaixe técnico da vaga; a página de experiências documenta o histórico linha a linha. As três, lidas juntas, formam o que um currículo de sênior faria numa única página — ou, seguindo a régua sem fonte primária que a nota 03 já registrou, em duas.

> [!example] Caso real
> Montagem de 2026-08-20 a partir de três fontes do site pessoal do autor deste vault: a página inicial em [josenaldo.com.br](https://josenaldo.com.br) (versão em português, `/pt`) e a página de experiências em [josenaldo.com.br/experiences](https://josenaldo.com.br/experiences), ambas verificadas ao vivo na mesma data; e o conteúdo de contratação, retirado do arquivo `content/pages/pt/hiring.md` do repositório do site — o site está em reformulação e essa página não está publicada no domínio ao vivo no momento desta escrita. Frases entre aspas são citações literais; o restante é síntese factual construída a partir dessas mesmas fontes.
>
...
Este documento merece o mesmo exame crítico que qualquer um dos anteriores, porque montá-lo a partir de três páginas diferentes, em vez de um único arquivo, é uma escolha que precisa ser justificada, não escondida. A [[03-Dominios/Carreira/Currículo/20 - A âncora|nota 20]] já registrou que a frase-âncora do currículo-base privado do autor — "I build delivery machines." — não tem link público disponível, e que a mesma posição aparece reformulada, em segunda pessoa, na página inicial do site. O que este capstone acrescenta é uma terceira renderização, encontrada no conteúdo de contratação do repositório do site: "Eu construo máquinas de entrega." — quase idêntica à frase privada que a nota 20 registrou sem poder linkar, só que esta versão está escrita, em português, num arquivo já pronto para publicação. As três frases não competem entre si; são a mesma âncora, vista de três ângulos, exatamente como a nota 20 já descreveu: uma declaração sobre si (o currículo-base), uma promessa ao leitor (a página inicial), e um argumento de contratação (o conteúdo de `/hiring`) — três saídas, uma origem, nenhuma contradição de substância entre elas.
...
graph LR
    ...
    Tra -.prova.-> P2["adaptação<br/>rápida"]
    Jr -.prova.-> P3["execução<br/>confiável"]
    Pl -.prova.-> P4["autonomia,<br/>owner"]
    Sr -.prova.-> P5["decisão de<br/>arquitetura"]
    St -.prova.-> P6["influência sem<br/>cargo formal"]

    Anc["a mesma pessoa,<br/>o mesmo tipo de trabalho<br/>observado seis vezes"] -.atravessa.-> Est
    Anc -.atravessa.-> Sr
    Anc -.atravessa.-> St
...
- **Josenaldo Matos** — arquivo `content/pages/pt/hiring.md` do repositório público do site (o site está em reformulação e esta página específica não está publicada no domínio ao vivo no momento desta escrita), conferido em 2026-08-20. Fonte do sumário e dos números atualizados citados na peça de sênior; conteúdo verificado no arquivo do repositório, não numa página publicada.
