...
> [!example] Caso real
> O `injection-harness` do autor deste vault — uma aplicação full-stack de planejamento de chicotes de injeção eletrônica, com backend em NestJS, frontend em React/TypeScript, autenticação JWT e regras de negócio por nível de assinatura — é um repositório **privado**: nenhum leitor externo consegue abri-lo, com ou sem o link. É um caso que vale nomear explicitamente, porque a etiqueta de status honesto desta nota não cobre só produção-ou-não — cobre também visibilidade, e um repositório fechado é, para efeito de sinal, diferente de um repositório público sem deploy. Um projeto público sem URL de produção ainda deixa o leitor examinar o código, o histórico de commits e o README; um projeto privado não deixa nada disso acontecer — o código simplesmente não fala por si, por mais bem escrito que esteja atrás do portão fechado. Isso não desqualifica o projeto como assunto de currículo, mas muda o que precisa carregar o peso da prova: como o link não sustenta mais nada sozinho, a linha do currículo e a conversa de entrevista precisam descrever o desafio técnico com detalhe suficiente para que o leitor avalie a decisão sem nunca ver uma linha do código — as regras de domínio por camada de assinatura, a arquitetura desacoplada entre backend e frontend, o motivo de cada escolha. É o mesmo material que um README bom descreveria; só que, aqui, ele precisa morar inteiro no currículo e na cabeça de quem o escreveu, porque não há repositório aberto para completar o que a frase deixou faltando. (Verificado via API do GitHub em 2026-08-20: `visibility: private`.)
...
> [!example] Caso real
> O repositório `aprendendo-git-e-github`, do autor deste vault, ilustra o segundo componente do que a IA não saturou: uso real, ainda que modesto. É um roteiro de aprendizagem curado — guias, cursos em vídeo, folhas de referência e material de troubleshooting sobre Git e GitHub, organizado por nível de progressão, publicado tanto como repositório no GitHub quanto como página estática, com URL de produção pública. O README, porém, não segue a regra desta nota à risca: ele abre com uma saudação de tom pessoal — "Saudações, armengados e armengadas!" — e só nomeia o propósito do projeto no terceiro parágrafo, de forma indireta ("Se você chegou até aqui e se interessa por aprender git e Github, é porque..."). O título já deixa o assunto claro, e o restante do documento organiza o conteúdo por nível de forma útil — mas as primeiras linhas gastam a atenção do leitor em tom, não em resposta. É um README bom no que entrega depois da abertura, e um exemplo do que esta nota recomenda melhorar: a saudação poderia vir depois de uma frase direta sobre o que é e para quem serve, sem perder o tom pessoal que o resto do documento mantém. O desafio técnico do projeto não é sofisticação de infraestrutura — é curadoria e organização pedagógica de material disperso numa progressão coerente, mantida ao longo do tempo, com atualização iterativa em vez de um único despejo inicial. É um projeto que nenhuma ferramenta de geração de código produziria numa tarde, porque o valor dele não está no código — está na curadoria humana por trás da escolha do que incluir, o mesmo tipo de decisão visível que esta nota descreve como o que passou a distinguir. (Fonte: [github.com/josenaldo/aprendendo-git-e-github](https://github.com/josenaldo/aprendendo-git-e-github) e [josenaldo.com.br/aprendendo-git-e-github](https://josenaldo.com.br/aprendendo-git-e-github/), README conferido ao vivo em 2026-08-20.)
